...
void studentaccount()
{
    ...
    Vector_push_back(&student_mis, m);
...
void teacher_givePerformance()
{
    ...
    if (Vector_search(&student_username, s) != -1 && Vector_search(&student_mis, m) == Vector_search(&student_username, s))
    {
        ...
        Vector_push_back(&per_student, m);
        map_insert(individual, m, s2);
    }
    else
    {
        printf("Either of Student name or mis not valid.:\n");
...
void teacher_markAttendance()
{
    ...
    while (f)
    {
        printf(CYAN "Enter student mis : " RESET);
        char m[20];
        // clear_input_buffer();
        // fgets(s, sizeof(s), stdin);
        // s[strcspn(s, "\n")] = 0;
        clear_input_buffer();
        scanf("%s", m);
        if (Vector_search(&student_mis, m) != -1)
        {
            Vector_push_back(&attend_student, m);
            Vector_push_back(&attendence_date, date);
        }
        else
            printf(RED "Invalid Student mis\n" RESET);
...
void admin_seePerformance()
{
    ...
    if (num == 1)
    {
        ...
        if (map_find(individual, m))
            printf("%s\n", map_get(individual, m));
        else
...
void deletestudentaccount()
{
    ...
    printf("Enter password : ");
    scanf("%s", pass);
    if (Vector_search(&student_username, n) != -1 && Vector_search(&student_password, pass) == Vector_search(&student_mis, m))
    {
        Vector_erase_value(&student_username, n);
        Vector_erase_value(&student_mis, m);
...
void student_list()
{
    ...
    {
        printf(CYAN "%d.  %s    %s\n" RESET, i + 1, student_username.data[i], student_mis.data[i]);
    }
}
...
void insertStudentData(MYSQL *conn)
{
    for (int i = 0; i < Vector_size(&student_username); i++)
    {
        ...
        char *mis = student_mis.data[i];
        // char *password = student_password.data[i];

        char query[100];
        sprintf(query, "INSERT INTO students (username, mis) VALUES ('%s', '%s')", username, mis);
...
void insertAttendanceData(MYSQL *conn) {
    for (int i = 0; i < Vector_size(&attendence_date); i++) {
        ...
        char query[100];
        sprintf(query, "INSERT INTO attend (mis, date) VALUES ('%s', '%s')", student, date);
...
void insertPerformanceData(MYSQL *conn) {
    for (int i = 0; i < Vector_size(&performance); i++) {
        ...
        char *perf = Vector_at(&performance, i);

        char query[200]; // Increased buffer size to accommodate longer queries
        sprintf(query, "UPDATE students SET performance = '%s' WHERE mis = '%s'", perf, student);
...
void updateAttendanceinStudentTable(MYSQL *conn){
    if (mysql_query(conn, "UPDATE students s \
                           SET s.attendance = ROUND(( \
                               SELECT (COUNT(*) / (SELECT COUNT(DISTINCT date) FROM attend)) * 100 \
                               FROM attend a \
                               WHERE a.mis = s.mis \
                           ), 2)")) {
        fprintf(stderr, "%s\n", mysql_error(conn));
...
int main()
{
    ...
    do
    {
        ...
        case 3:
            ...
            if (Vector_search(&student_username, user) != -1 && Vector_search(&student_password, pass) == Vector_search(&student_mis, m))
            {
                int num;
                printf(GREEN "\n\n\nWelcome to Acoount\n" RESET);
                do
        ...
        case 4:
            ...
            updateAttendanceinStudentTable(conn);
